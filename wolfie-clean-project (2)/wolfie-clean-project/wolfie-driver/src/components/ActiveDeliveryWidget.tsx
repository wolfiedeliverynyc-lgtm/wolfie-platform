...
export default function ActiveDeliveryWidget({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const API_BASE = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/api/v1` : 'https://wolfie-backend-pt9u.onrender.com/api/v1';
  ...
  if (order.status === 'NAV_TO_STORE') {
    return (
      <div id="active-navigation-screen" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
        {/* HEADER BLOCK */}
        <div className="flex items-center justify-between pb-3 border-b border-slate-900">
          <div className="flex items-center gap-2">
            <button
              onClick={() => {}}
              className="w-10 h-10 rounded-2xl bg-bg-app border border-slate-850 flex items-center justify-center text-text-primary"
              title="Navigating"
            >
          ...
          <button
            className="px-3.5 py-1.5 rounded-full border border-slate-800 bg-bg-app/60 text-text-primary text-xs font-black uppercase tracking-wider hover:text-white transition-all cursor-pointer"
          >
            Navigate
          </button>
        </div>
        ...
        {/* PILL CARD BLOCK */}
        <div className="bg-bg-card p-4.5 rounded-[28px] border border-slate-900/60 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-primary/10 border border-primary/25 flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-[12px] text-text-secondary font-bold leading-tight mt-1">
          ...
          <div className="flex gap-2">
            <a 
              ...
              className="w-10 h-10 rounded-full bg-bg-card-hover border border-slate-850 hover:bg-bg-card flex items-center justify-center text-text-primary transition-all"
              title="Call Store"
            >
        ...
        <div className="h-[50vh] rounded-[28px] overflow-hidden border border-slate-900 shadow-inner relative flex-shrink-0">
          {mapNode}
        </div>
        ...
        <div className="flex flex-col py-0.5">
          ...
          <p className="text-[11px] font-black text-text-secondary uppercase tracking-widest mt-1.5">
        ...
        <div className="grid grid-cols-2 gap-3 pb-1">
          <button
            onClick={() => {}}
            className="py-3.5 bg-bg-card-hover hover:bg-bg-card-hover border border-slate-900/60 rounded-[20px] text-[11.5px] font-black uppercase tracking-wider text-text-primary flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
  ...
  if (order.status === 'NAV_TO_CUSTOMER') {
    ...
    return (
      <div id="active-navigation-customer-screen" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
        {/* HEADER BLOCK */}
        <div className="flex items-center justify-between pb-3 border-b border-slate-900 justify-start">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-bg-app flex items-center justify-center text-[#23a24d]">
          ...
          <button
            ...
            className="px-4 py-1.5 rounded-xl border border-slate-800 bg-bg-card hover:bg-bg-card-hover text-text-primary text-xs font-extrabold tracking-wide hover:border-slate-700 transition-all cursor-pointer"
        ...
        <div className="bg-bg-card p-4 rounded-[28px] border border-slate-900/60 flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-primary/10 border border-primary/25 flex items-center justify-center shrink-0">
            <div className="w-8 h-8 rounded-full bg-primary text-black flex items-center justify-center">
              <MapPin className="w-4 h-4 text-white" />
            </div>
          ...
          <div>
            ...
            <p className="text-[12px] text-text-primary font-bold leading-tight mt-1">
        ...
        {/* INTEGRATED MAP CONTAINER */}
        <div className="flex-1 min-h-[50vh] rounded-[28px] overflow-hidden border border-slate-900 shadow-inner relative flex-shrink-0">
          {mapNode}
        </div>

        {/* HUD DISTANCE & ESTIMATED ARRIVAL METRICS */}
        <div className="bg-bg-app border border-slate-900/85 rounded-[24px] p-4.5 flex items-center justify-between">
          <div className="flex flex-col">
            <h3 className="text-2xl font-black text-white font-sans tracking-tight leading-none">
              {navigationDistanceLeft.toFixed(1)} km
            </h3>
            <p className="text-xs font-extrabold text-text-secondary mt-1.5">
              {remainingMinutes} min away
            </p>
          </div>
          <div className="text-right flex flex-col justify-end">
            <span className="text-[10px] font-black text-text-secondary uppercase tracking-wider leading-none">
              Estimated Arrival
            </span>
        ...
        <div className="pt-1">
          <button
            ...
            className="w-full h-14 bg-transparent border border-slate-850 hover:bg-bg-card hover:border-slate-700 active:scale-[0.99] rounded-[20px] text-text-primary text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
          >
            <Phone className="w-4 h-4 text-text-secondary fill-slate-400/10" />
  ...
  if (order.status === 'ARRIVED_AT_STORE' && !arrivalConfirmed) {
    return (
      <div id="arrived-restaurant-page" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
        {/* HEADER BLOCK */}
        <div className="flex items-center gap-2 pb-3 border-b border-slate-900 justify-start">
          <div className="w-10 h-10 rounded-2xl bg-bg-app border border-emerald-500/20 flex items-center justify-center text-emerald-400">
        ...
        <div className="bg-bg-card p-4.5 rounded-[28px] border border-slate-900/60 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <a 
              href={`tel:555-${order.orderNumber}`}
              onClick={(e) => e.preventDefault()}
              className="w-10 h-10 rounded-full bg-bg-card-hover border border-slate-850 hover:bg-bg-card flex items-center justify-center text-text-primary transition-all cursor-pointer"
              title="Call Store"
            >
              <Phone className="w-4 h-4" />
            </a>
        ...
        <div className="bg-bg-app border border-slate-900/85 rounded-[24px] p-4.5 flex flex-col space-y-4">
          ...
          <div className="space-y-3.5 pt-1">
            ...
            <div className="pt-2 flex items-center justify-between">
              <button
                ...
                className="px-4 py-2 border border-slate-800 hover:border-slate-700 bg-bg-app hover:bg-bg-card text-[11px] font-extrabold rounded-xl text-text-primary transition-all cursor-pointer"
              >
            ...
            {showItemsBeforeArrival && (
              <div className="bg-bg-card p-3 rounded-2xl border border-slate-850/50 space-y-2 mt-2 animate-[fadeIn_0.2s_ease-out]">
                {order.items.map(item => (
                  <div key={item.id} className="flex justify-between text-[11px] font-bold text-text-secondary">
        ...
        <div className="pt-4 flex flex-col space-y-4">
          {notifiedMerchantWait && (
            <div className="p-3 bg-orange-650/10 border border-primary/20 text-primary text-xs font-bold rounded-xl text-center animate-pulse">
          ...
          <div className="text-center py-2">
            <span className="text-xs font-bold text-text-secondary">Waiting for order? </span>
            <button
              onClick={() => setNotifiedMerchantWait(true)}
              className="text-xs font-black text-primary hover:text-primary transition-colors underline bg-transparent border-none p-0 cursor-pointer"
            >
              Tell us
            </button>
          </div>
  ...
  if (showDeliveredPage) {
    ...
    return (
      <div id="delivered-state-page" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
        {/* HEADER BLOCK */}
        <div className="relative flex items-center justify-between pb-3 border-b border-slate-900">
          <button
            ...
            className="w-10 h-10 rounded-2xl bg-bg-app border border-slate-850 flex items-center justify-center text-text-primary hover:bg-bg-card transition-all cursor-pointer"
            title="Back"
          >
            <ArrowLeft className="w-5 h-5 text-text-primary" />
          </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-6 relative overflow-hidden">
          ...
          <div className="absolute w-44 h-44 rounded-full border border-primary/10 animate-ring-expand [animation-delay:0.75s] pointer-events-none"></div>
        ...
        <div className="bg-bg-card border border-slate-900/80 rounded-[28px] p-5.5 space-y-4">
          ...
          <div className="space-y-3 pt-0.5 text-xs font-bold">
            <div className="flex justify-between items-center">
              <span className="text-text-secondary font-medium">Base Pay</span>
              <span className="text-text-primary font-mono">${(isPizzaPalaceTrip ? 6.00 : displayBasePay).toFixed(2)}</span>
            </div>

            <div className="flex justify-between items-center">
              <span className="text-text-secondary font-medium">Distance Pay</span>
              <span className="text-text-primary font-mono">${(isPizzaPalaceTrip ? 1.75 : displayDistancePay).toFixed(2)}</span>
            </div>

            <div className="flex justify-between items-center">
              <span className="text-text-secondary font-medium">Tip</span>
              <span className="text-text-primary font-mono">${(isPizzaPalaceTrip ? 1.00 : adjustedTip).toFixed(2)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full h-14 bg-accent text-white hover:brightness-110 active:scale-[0.99] rounded-[20px] text-white text-sm font-black uppercase tracking-widest transition-all flex items-center justify-between px-6 cursor-pointer shadow-lg shadow-orange-950/20"
          >
  ...
  if (order.status === 'ARRIVED_AT_STORE' && pickupConfirmed) {
    return (
      <div id="order-picked-up-page" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
        {/* HEADER BLOCK */}
        <div className="flex items-center gap-2 pb-3 border-b border-slate-900 justify-start">
          <div className="w-8 h-8 rounded-full bg-bg-card-hover border border-emerald-500/20 flex items-center justify-center text-emerald-400">
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-6 relative overflow-hidden">
          ...
          <div className="relative w-36 h-36 mx-auto rounded-full border border-emerald-500/25 flex items-center justify-center bg-[#0b1413] animate-bounce-in shadow-xl shadow-emerald-950/10">
            ...
            <div className="relative p-6 bg-primary/10 border border-primary/20 rounded-3xl">
              <ShoppingBag className="w-14 h-14 text-primary fill-orange-500/10" />
        ...
        <div className="bg-bg-app border border-slate-900/85 rounded-[24px] p-4.5 flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className="px-2 py-2 bg-[#171110] rounded-xl flex items-center justify-center text-primary border border-primary/10 mt-0.5">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs font-bold text-text-secondary mt-1 leading-tight">
                {customerAddressParts.street}
              </p>
              <p className="text-[10px] text-text-secondary font-mono font-bold mt-0.5">
                {customerAddressParts.city}
              </p>
            </div>
          </div>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full h-14 bg-accent text-white hover:brightness-110 active:scale-[0.99] rounded-[20px] text-white text-center text-xs font-black uppercase tracking-widest cursor-pointer shadow-lg shadow-orange-950/20 hover:shadow-orange-900/35 transition-all flex items-center justify-center gap-2"
          >
  ...
  return (
    <div id="active-delivery-column" className="bg-bg-card border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col justify-between h-full">
      {/* BRIEF CARD HEADER */}
      <div className="bg-bg-app p-4 border-b border-slate-850 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-[10px] text-text-secondary font-bold uppercase tracking-wider block">Active Dispatch Route</span>
            <div className="flex items-center gap-1.5">
              <h4 className="font-bold text-xs text-text-primary">
                {order.status.includes('STORE') ? order.storeName : order.customerName}
              </h4>
              <span className="text-[10px] text-text-secondary font-mono font-bold bg-bg-card border border-slate-800 px-1 py-0.2 rounded">
                #{order.orderNumber}
              </span>
            </div>
          </div>
        ...
        <div className="flex gap-1.5">
          <a
            href={`tel:555-${order.orderNumber}`}
            onClick={(e) => e.preventDefault()}
            className="w-8 h-8 bg-bg-card hover:bg-slate-805 rounded-lg border border-slate-800 flex items-center justify-center text-text-secondary hover:text-primary transition-colors"
            title="Call"
          >
            <Phone className="w-4 h-4" />
          </a>
      ...
      <div className="p-5 flex-1 overflow-y-auto space-y-4">
        ...
        {order.status === 'ARRIVED_AT_STORE' && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-bg-app p-4 rounded-xl border border-slate-850 space-y-2">
              <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest">Merchant Instructions</p>
              <p className="text-xs font-semibold text-text-primary">
                "Tell cashier you are and pickup order for <strong className="text-primary">#{order.orderNumber}</strong>. Ensure all drinks and sauces are collected!"
              </p>
            </div>

            {/* Dynamic Items checklists */}
            <div className="space-y-2">
              <p className="text-xs font-bold text-text-secondary uppercase tracking-widest text-[9px]">Checklist ({order.items.filter(i=>i.checked).length}/{order.items.length})</p>
              <div className="space-y-2">
                {order.items.map((item) => (
                  <label
                    key={item.id}
                    ...
                    className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${item.checked ? 'bg-emerald-500/5 border-emerald-500/20 text-text-primary' : 'bg-bg-app hover:bg-slate-850 border-slate-850 text-text-secondary'}`}
                  >
                    <div className="flex items-center gap-2 text-xs font-medium">
                      <span className="text-[10px] font-bold text-text-secondary font-mono bg-bg-card border border-slate-800 px-1.5 py-0.5 rounded">
                        QTY: 1
                      </span>
                      <span>{item.name}</span>
                    </div>
        ...
        {order.status === 'ARRIVED_AT_CUSTOMER' && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-bg-app p-4 rounded-xl border border-slate-850 space-y-1">
              <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest">Dropoff Instructions</p>
            ...
            <div className="space-y-2">
              ...
              {simulatedPhotoUrl ? (
                ...
                <div className="border border-dashed border-slate-800 rounded-2xl h-[130px] flex flex-col items-center justify-center p-4 bg-bg-app hover:bg-bg-app/80 transition-colors">
                  <Camera className="w-8 h-8 text-slate-600 mb-1" />
                  <p className="text-[10px] text-text-secondary font-medium font-sans">Required visual drops verification</p>
                </div>
              )}
              ...
              <div className="relative">
                  ...
                  />
                  <div
                    className={`w-full py-2.5 rounded-xl text-xs font-bold border flex items-center justify-center gap-1.5 transition-all ${capturing ? 'bg-bg-app border-slate-850 text-text-secondary' : 'bg-bg-app hover:bg-slate-850 border-slate-800 text-text-primary'}`}
                  >
                    {capturing ? (
      ...
      <div className="p-4 bg-bg-app border-t border-slate-850">
        {order.status === 'ARRIVED_AT_STORE' && (
          <button
            id="pickup-confirm"
            onClick={() => setPickupConfirmed(true)}
            disabled={!allItemsChecked}
            className={`w-full py-3 rounded-xl text-xs font-bold font-sans flex items-center justify-center gap-1.5 transition-all ${allItemsChecked ? 'bg-primary text-black hover:bg-primary text-black text-text-primary border border-primary/20 shadow-lg shadow-primary/10 cursor-pointer' : 'bg-slate-850 border-slate-800 text-text-secondary cursor-not-allowed'}`}
          >
        ...
        {order.status === 'ARRIVED_AT_CUSTOMER' && (
          <button
            ...
            className={`w-full py-3 rounded-xl text-xs font-bold font-sans flex items-center justify-center gap-1.5 transition-all ${photoCaptured ? 'bg-emerald-600 hover:bg-emerald-500 text-text-primary border border-emerald-500/20 shadow-lg shadow-emerald-500/10 cursor-pointer' : 'bg-slate-850 border-slate-800 text-text-secondary cursor-not-allowed'}`}
          >
            <Check className="w-4 h-4" />
