...
class WAPEngine:
    ...
    def __init__(self, mapbox: MapboxClient = None):
        self.mapbox = mapbox
        self._model_trained = True  # Always True since we use heuristics/rules
    ...
    def _predict_prep_time(self, restaurant_id: str, item_count: int,
        ...
        # Heuristic adjustment if enough data points exist
        if profile.get('data_points', 0) >= MIN_DATA_POINTS:
            variance = profile.get('variance', 5.0)
            # Add a small buffer proportional to historical preparation variance
            base_time = profile.get('avg_prep_time', 15.0) + (variance * 0.1)
    ...
    def _retrain_model(self, session, restaurant_id: str):
        """Update heuristics profile with new data (ML-free)."""
        self._model_trained = True
        print(f"[WAP] Heuristics profile updated for restaurant {restaurant_id}")
