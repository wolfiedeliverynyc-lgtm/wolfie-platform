...
export default function HomePage() {
  ...
  const [currentView, setCurrentView] = useState<'onboarding' | 'login' | 'register' | 'otp' | 'forgot' | 'reset' | 'address_entry' | 'home' | 'detail' | 'cart' | 'checkout' | 'tracking' | 'restaurant' | 'chat'>('onboarding');
    ...
    return (
      <div className="max-w-[1400px] mx-auto flex gap-8 select-none animate-fadeIn text-left py-6 px-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {activeRestaurantList.map(res => (
                <div 
                  key={res.id}
                  onClick={() => { setSelectedRestaurant(res); setPreviousView('home'); setCurrentView('restaurant'); }}
                ...
                >
                  <div className="h-[140px] relative overflow-hidden bg-gray-150">
                    <img src={res.cover} alt={res.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    {res.isBestSeller && (
                      <span className="absolute left-4 top-4 bg-[#FFE100] text-[#3C2F2F] font-roboto font-black text-[11px] px-3 py-1 rounded-full uppercase tracking-wider shadow">Best Seller</span>
                    )}
                  </div>
                  <div className="p-5 flex gap-3.5 relative">
                    ...
                    <div className="text-left flex-1">
                      <h4 className="font-poppins font-bold text-[16px] text-[#3C2F2F] group-hover:text-[#EF2A39] transition-colors">{res.name}</h4>
                      <p className="font-roboto text-[12px] text-[#A6A6A6] mt-0.5">{res.tags.join(' • ')}</p>
                      <div className="flex items-center gap-3.5 mt-2.5 font-roboto text-[12px] font-bold text-[#6A6A6A]">
                        <span className="flex items-center gap-0.5 text-yellow-500">★ {res.rating}</span>
                        <span>• {res.distance} miles</span>
          ...
          {/* Dishes by Category (Multi-shelves) */}
          <div className="space-y-10">
                ...
                <div key={catName} className="animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {categoryItems.map(item => (
                      <div 
                        key={item.id}
                        onClick={() => { setSelectedFoodItem(item); setPreviousView('home'); setCurrentView('detail'); }}
            ...
            {/* Check if no items match filters across all categories */}
  ...
  const renderDesktopSettings = () => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          <span className="font-poppins font-bold text-[15.5px] text-[#3C2F2F] block">
                      ...
                      <button 
                        ...
                        className="text-gray-400 hover:text-[#EF2A39] hover:bg-red-50 p-2.5 rounded-full transition-colors focus:outline-none cursor-pointer"
                      >
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                          <polyline points="3 6 5 6 21 6" />
                          <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                    ...
                    orders.filter(o => o.status !== 'Completed').map(order => (
                      <div key={order.id} className="border border-gray-100 bg-white rounded-[24px] p-5.5 shadow-sm text-left relative flex flex-col justify-between min-h-[170px]">
                        ...
                        <div className="flex justify-between items-center border-t border-gray-50 pt-3.5 mt-3.5">
                          <span className="font-poppins font-black text-[16px] text-[#EF2A39]">${order.totalPrice.toFixed(2)}</span>
                          <button 
                            onClick={() => {
                              setOrderedItems(order.items);
                              setCurrentView('tracking');
                  ...
                  {orders.filter(o => o.status === 'Completed').length === 0 ? (
                    ...
                    orders.filter(o => o.status === 'Completed').map(order => (
                      <div key={order.id} className="border border-gray-100 bg-white rounded-[24px] p-5.5 shadow-sm text-left relative flex flex-col justify-between min-h-[170px]">
                        ...
                        <div className="flex justify-between items-center border-t border-gray-50 pt-3.5 mt-3.5">
                          <span className="font-poppins font-black text-[16px] text-[#3C2F2F]">${order.totalPrice.toFixed(2)}</span>
                          <button 
                            onClick={() => {
    ...
    return (
      <div className="max-w-[1400px] mx-auto select-none animate-fadeIn text-left flex gap-8 py-6 px-4">
    ...
    return (
      <div className="max-w-[1400px] mx-auto select-none animate-fadeIn text-left pb-16">
        ...
        <div className="flex gap-6 items-end mb-8 relative">
          ...
          <div className="flex-1 text-left">
            <div className="flex items-center gap-2">
              <h2 className="font-poppins font-bold text-[32px] text-[#3C2F2F] leading-none">{selectedRestaurant.name}</h2>
            ...
            <p className="font-roboto text-[14px] text-[#A6A6A6] mt-2">{selectedRestaurant.tags.join(' • ')}</p>
          </div>
            ...
            {restaurantTab === 'overview' && (
              <div className="bg-white border border-gray-100 rounded-[28px] p-6 shadow-sm space-y-6">
                <div>
                  ...
                  <p className="font-roboto text-[14.5px] text-[#6A6A6A] leading-relaxed">{selectedRestaurant.description}</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-center border-t border-gray-100 pt-6">
                  <div className="space-y-1">
                    ...
                    <span className="text-[15px] font-poppins font-bold text-[#3C2F2F] block">{selectedRestaurant.deliveryTime}</span>
                  </div>
                  <div className="space-y-1 border-x border-gray-100">
                    ...
                    <span className="text-[15px] font-poppins font-bold text-[#3C2F2F] block">${selectedRestaurant.deliveryFee.toFixed(2)}</span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[11px] font-bold text-[#A6A6A6] uppercase tracking-wider block">Min Order</span>
                    <span className="text-[15px] font-poppins font-bold text-[#3C2F2F] block">${selectedRestaurant.minOrder.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {restaurantTab === 'menu' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                  {['All', 'Burgers', 'Chicken', 'Drinks'].map(cat => (
                    <button
                      key={cat}
              ...
              <div className="bg-white border border-gray-100 rounded-[28px] p-6 shadow-sm space-y-6 animate-fadeIn">
                <div className="flex gap-6 items-center border-b border-gray-50 pb-6">
                  ...
                  <div className="text-left flex-1">
                    ...
                    <p className="font-roboto text-[12px] text-[#A6A6A6] mt-0.5">Showing latest reviews for {selectedRestaurant.name}</p>
                  </div>
                  <div className="bg-[#FFE100]/10 border border-[#FFE100]/25 rounded-2xl p-4 text-center">
                    <span className="font-poppins font-black text-[22px] text-[#3C2F2F] leading-none block">{selectedRestaurant.rating}</span>
  ...
  const renderDesktopDetail = () => {
    ...
    const otherMenuItems = activeDishes.filter(item => item.id !== selectedFoodItem.id);
    ...
    const getCartItemQuantity = (itemId: string) => {
      const matches = cartItems.filter(ci => ci.foodItem.id === itemId);
    ...
    const adjustCartItemQuantityDirect = (item: FoodItem, delta: number) => {
      ...
      if (existingIndex > -1) {
        updateQuantity(cartItems[existingIndex].cartId, delta);
      } else if (delta > 0) {
        addToCartDirect(item);
      }
    };

    const activeNutrition = getNutritionValues(selectedFoodItem.id);
    ...
    return (
      <div className="max-w-[1400px] mx-auto select-none animate-fadeIn text-left flex flex-col lg:flex-row gap-8 py-6 px-4">
        ...
        <div className="flex-1 bg-white border border-gray-100 rounded-[32px] p-8 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-[13px] font-roboto font-bold text-gray-500 bg-gray-50 px-3 py-1.5 rounded-full shadow-xs border border-gray-100">{activeNutrition.kcal}</span>
            </div>
            ...
            <div className="relative flex items-center justify-center h-[260px] w-full mb-6 bg-gray-50/40 rounded-[28px] p-6">
              <img src={selectedFoodItem.image} alt={selectedFoodItem.name} className="max-h-[220px] max-w-[220px] object-contain hover:scale-105 transition-transform duration-300" />
            </div>

            {/* Nutrition Pills */}
            <div className="flex gap-2.5 flex-wrap my-5 select-none">
              <span className="font-roboto font-bold text-[12px] text-gray-500 bg-gray-50 border border-gray-100/70 px-4 py-1.5 rounded-full">Fat: {activeNutrition.fat}</span>
              <span className="font-roboto font-bold text-[12px] text-gray-500 bg-gray-50 border border-gray-100/70 px-4 py-1.5 rounded-full">Saturates: {activeNutrition.saturates}</span>
              <span className="font-roboto font-bold text-[12px] text-gray-500 bg-gray-50 border border-gray-100/70 px-4 py-1.5 rounded-full">Sugars: {activeNutrition.sugars}</span>
              <span className="font-roboto font-bold text-[12px] text-gray-500 bg-gray-50 border border-gray-100/70 px-4 py-1.5 rounded-full">Salt: {activeNutrition.salt}</span>
            </div>

            {/* Item Info */}
            <div className="space-y-4">
              <div>
                <span className="font-roboto font-bold text-[12.5px] text-[#EF2A39] uppercase tracking-[0.15em] block">{selectedFoodItem.brand}</span>
                <h2 className="font-poppins font-bold text-[30px] text-[#3C2F2F] mt-1 mb-2 leading-tight">{selectedFoodItem.name}</h2>
                <div className="flex items-center gap-2 text-[#FFE100] text-[14.5px]">
                  <span>★ {selectedFoodItem.rating.toFixed(1)}</span>
                  <span className="text-gray-300 font-normal">•</span>
                  <span className="font-roboto text-[13px] font-bold text-[#A6A6A6]">{selectedFoodItem.deliveryTime} delivery speed</span>
                </div>
              </div>

              <p className="font-roboto text-[15px] text-[#6A6A6A] leading-relaxed">{selectedFoodItem.description}</p>
          ...
          <div className="flex items-center gap-5 pt-6 mt-6 border-t border-gray-100">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl px-5 py-2 text-center shrink-0">
              ...
              <span className="font-poppins font-black text-[22px] text-[#3C2F2F]">
                ${(
                  ...
                  selectedDrinks.reduce((sum, id) => sum + (drinkOptions.find(o => o.id === id)?.price || 0), 0)) * portionCount
                ).toFixed(2)}
              </span>
            </div>
            ...
            <div className="flex-1 flex gap-3.5">
              <button 
                onClick={addToCart}
                className="flex-1 h-[54px] bg-[#FFE100] hover:brightness-95 active:scale-98 text-[#3C2F2F] font-roboto font-bold text-[15px] rounded-[16px] transition-all cursor-pointer focus:outline-none shadow-sm flex items-center justify-center"
              >
                {addedToCartFeedback ? "✓ ADDED TO BASKET!" : "ADD TO BASKET"}
              </button>
              <button 
                onClick={() => { setPreviousView('detail'); setCurrentView('cart'); }}
        ...
        {/* Modal Customizer Popup for Desktop */}
        {activeCustomizerTab && (
                ...
                {cartItems.map(item => (
                  <div key={item.cartId} className="flex gap-4 items-center border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="font-roboto text-[11px] text-[#A6A6A6] mt-0.5">{item.foodItem.brand} • Size: {item.size} • Spicy: {item.spicy}%</p>
    ...
    return (
      <div className="max-w-[1400px] mx-auto select-none animate-fadeIn text-left pb-16 flex flex-col lg:flex-row gap-8 min-h-[calc(100vh-140px)]">
            ...
            <div className="bg-white border border-gray-100 rounded-[28px] p-6 shadow-sm space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <span className="font-roboto text-[11px] font-bold text-[#A6A6A6] uppercase tracking-wider block mt-0.5">Order ID: #{orderedItems.length > 0 ? 'WOLF_983210' : 'WOLF_193240'}</span>
                </div>
              </div>
              ...
              <div className="flex gap-4 border-t border-gray-50 pt-4 items-center justify-between">
                <div className="text-left">
                  ...
                  <span className="text-[24px] font-poppins font-black text-[#3C2F2F] mt-1 block">12 Minutes</span>
                </div>
        ...
        <div className="flex-1 bg-white border border-gray-100 rounded-[32px] overflow-hidden shadow-sm min-h-[50vh] relative">
          <div ref={desktopMapContainerRef} className="w-full h-full min-h-[50vh]" />
