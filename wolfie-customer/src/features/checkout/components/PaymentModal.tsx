...
import { CreditCard, Clipboard, ShieldCheck, ShoppingBag, ArrowRight, CheckCircle2, RefreshCw, X, MapPin } from 'lucide-react';
import { Address, PaymentMethod, Restaurant } from '../../../lib/types';
...
export default function PaymentModal({
  ...
  const { cart, getCartSubtotal, cartNote } = useCartStore();
  const { addresses, paymentMethods } = useAuthStore();
  ...
  const [deliveryNotes, setDeliveryNotes] = useState(cartNote);
  const [isDetectingLocation, setIsDetectingLocation] = useState(false);

  const handleDetectCurrentLocationCheckout = () => {
      ...
      async (position) => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            const city = addr.city || addr.town || addr.village || 'New York';
            const state = addr.state || 'NY';
            const zip = addr.postcode || '10011';
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 select-none">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 tracking-tight font-sans text-left">Secure Checkout</h3>
                    <p className="text-xs text-slate-400 font-bold font-sans mt-0.5 text-left">Order from {restaurant.name}</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
                  className="text-slate-400 hover:text-slate-800 p-2 hover:bg-slate-55 rounded-xl transition-all cursor-pointer"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-5 text-slate-800">
                  {/* Choose Address node */}
                  <div>
                    <label className="flex items-center justify-between text-[10px] uppercase font-black tracking-wider text-slate-400 mb-2.5 text-left select-none">
                      <span>Select Delivery Address</span>
                      <button
                        type="button"
                        onClick={handleDetectCurrentLocationCheckout}
                        disabled={isDetectingLocation}
                        className="text-[10px] text-[#F15A24] hover:text-[#E04D1B] font-extrabold flex items-center gap-1 cursor-pointer transition-colors"
                      >
                      ...
                      {addresses.map((addr) => (
                        ...
                        >
                          <input
                            ...
                            className="accent-[#F15A24] h-4 w-4 mt-0.5 cursor-pointer text-[#F15A24] focus:ring-[#F15A24]"
                  ...
                  {/* Choose Payment node */}
                  <div>
                    <label className="block text-[10px] uppercase font-black tracking-wider text-slate-400 mb-2.5 text-left">
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-black tracking-wider text-slate-400 mb-2 flex items-center gap-1.5 select-none text-left">
                ...
                <div className="bg-[#FCFCFD] border border-[#ECEFF2] p-6 rounded-3xl flex flex-col justify-between">
                  <div>
                    <h4 className="text-[10px] uppercase font-black tracking-wider text-slate-400 border-b border-slate-105 pb-2.5 select-none text-left">
                    ...
                    <div className="max-h-36 overflow-y-auto space-y-2 py-3 border-b border-slate-100/80 mb-4 scrollbar-none select-none">
                      {cart.map((item) => {
                        ...
                        return (
                          <div key={item.cartItemId} className="flex flex-col text-xs text-slate-700 py-1.5 border-b border-slate-100/40 last:border-0 last:pb-0">
                            <div className="flex justify-between items-center">
                              <span className="truncate max-w-[170px] font-black text-slate-800 font-sans text-left">
                                {item.quantity}× {item.menuItem.name}
                              </span>
                              <span className="font-mono text-slate-900 font-bold">${(unitPrice * item.quantity).toFixed(2)}</span>
                            </div>
                            {item.customization && (
                              <span className="text-[10px] font-semibold text-slate-400 mt-1 leading-tight font-sans block text-left">
                  ...
                  <div className="pt-6 space-y-3.5 text-center">
                    <button
