...
export default function DriverCompanion() {
  ...
  const { userEmail, getDefaultAddress, getDefaultPaymentMethod } = useAuthStore();
  ...
  // Mutation to auto-generate a demo order for instant driver simulation
  const handleAutoPlaceDemoOrder = async () => {
    try {
      const list = await restaurantApi.getRestaurants();
      const demoRestaurant = list[0];
      ...
      startOrderSimulation(demoOrder);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto px-4 py-8 text-slate-800 font-sans" id="driver-app-layout">
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        {/* System Guidance */}
        <div className="bg-white rounded-3xl p-6 border border-[#ECEFF2] shadow-xs select-none text-left">
      ...
      <div className="lg:col-span-8 space-y-6">
        {activeOrder ? (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[2rem] p-6 border border-[#ECEFF2] shadow-[0_8px_30px_rgba(0,0,0,0.02)] text-left">
              ...
              {statusAction ? (
                <div className="pt-2">
                  ...
                  <button
                    onClick={() => updateOrderStatus(statusAction.next)}
            ...
            <div className="bg-white border border-[#ECEFF2] rounded-[2rem] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col h-[400px]" id="driver-chat-container">
              ...
              <div className="mb-3 select-none text-left">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
                  {templates.map((tmpl, idx) => (
                    <button
                      key={idx}
                      onClick={() => addMessage(tmpl, 'rider')}
                      className="py-1.5 px-3 bg-slate-50 hover:bg-orange-50 hover:text-[#F15A24] border border-slate-105 hover:border-orange-200 text-slate-600 text-[10px] font-bold rounded-xl whitespace-nowrap cursor-pointer transition-all"
                    >
                      Ping Mode #{idx + 1}
                    </button>
                  ))}
          ...
          <div className="bg-white rounded-[2rem] p-10 border border-[#ECEFF2] text-center space-y-6 shadow-sm select-none" id="driver-no-orders">
            <div className="w-20 h-20 bg-amber-50 rounded-full flex items-center justify-center mx-auto relative border border-amber-100">
              <span className="absolute inset-0 bg-amber-100/30 rounded-full animate-ping" />
              <Bike className="w-10 h-10 text-amber-550 stroke-[2]" />
            </div>

            <div className="max-w-md mx-auto space-y-2">
              <h3 className="text-xl font-black text-slate-900 font-sans tracking-tight">On Guard. Scanning for Jobs...</h3>
              <p className="text-xs text-slate-400 font-bold leading-relaxed">
                Coordinates are clear. Checkout first as a Customer, or tap below to instantly generate a simulated gourmet meal job.
              </p>
            </div>

            <div className="pt-4 max-w-sm mx-auto">
              <button
                onClick={handleAutoPlaceDemoOrder}
                className="w-full py-4 bg-[#FFD54F] hover:bg-[#FBC02D] text-slate-950 text-xs font-black tracking-widest uppercase rounded-2xl shadow-md transition-transform active:scale-98 flex items-center justify-center space-x-2 cursor-pointer font-sans"
              >
