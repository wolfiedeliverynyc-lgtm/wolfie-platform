...
export default function App() {
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    return (localStorage.getItem('dasher_theme') as 'dark' | 'light') || 'dark';
  });

  useEffect(() => {
    localStorage.setItem('dasher_theme', theme);
    if (theme === 'light') {
  ...
  const handleCompleteActiveDelivery = (proofPhotoUrl: string) => {
    ...
    try {
      ...
      fetch(`${apiUrl}/orders/${activeOrder.id}/status`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div className={`min-h-screen bg-[#050611] text-slate-100 font-sans antialiased flex flex-col items-center justify-center py-0 md:py-6 bg-[radial-gradient(ellipse_85%_85%_at_50%_-20%,rgba(255,85,0,0.12),rgba(255,255,255,0))] select-none ${theme === 'light' ? 'light-theme' : ''}`}>
      
      {/* Floating Theme Control Panel (Desktop) */}
      <div className="fixed top-6 right-6 z-[999] hidden md:flex flex-col gap-3 bg-slate-900 border border-slate-800 p-4 rounded-3xl shadow-xl">
        <h4 className="text-[10px] font-black text-slate-400 tracking-widest uppercase">Phone View Mode</h4>
        <button
          onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
          className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-slate-950 text-xs font-bold text-white hover:bg-slate-850 transition-all cursor-pointer border border-slate-850"
        >
          {theme === 'light' ? (
            <>
      ...
      {/* Floating Theme Control Button (Mobile) */}
      <button
        onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
        className="fixed top-4 right-4 z-[999] md:hidden flex items-center justify-center w-10 h-10 rounded-full bg-slate-900 border border-slate-800 text-white shadow-lg cursor-pointer"
      >
        {theme === 'light' ? <Moon className="w-4 h-4 text-orange-500" /> : <Sun className="w-4 h-4 text-orange-400" />}
      </button>
