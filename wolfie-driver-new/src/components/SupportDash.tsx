...
export default function SupportDash({
  ...
  const quickHelpTopics = [
    { id: 'issue_order', title: 'I have an issue with my order', icon: <UtensilsCrossed className="w-4 h-4 text-primary" />, initialResponse: "Hi there! I can help with order details. If this is an active order, please select what's wrong: Is a customer item missing, is the food damaged, or has there been an issue or delay with preparing the food at the merchant site?" },
  ...
  return (
    <div id="support-panel" className="flex flex-col flex-1 h-full text-text-primary font-sans animate-[fadeIn_0.3s_ease-out] space-y-4 pb-2">
      <div className="relative flex items-center justify-between pb-3 border-b border-slate-900">
        <button onClick={() => { triggerSound('CLICK'); if (activeView === 'CHAT') { setActiveView('MAIN'); } else { onBack(); } }} className="w-10 h-10 rounded-2xl bg-bg-card border border-slate-900 flex items-center justify-center text-text-primary hover:bg-bg-card transition-all cursor-pointer shadow-md">
      ...
      {activeView === 'MAIN' ? (
        <div className="space-y-4.5 flex flex-col flex-1 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-bg-card border border-slate-900 rounded-[28px] p-5.5 flex items-center gap-4.5 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-28 h-28 bg-accent text-white/5 rounded-full blur-2xl pointer-events-none"></div>
            <div className="w-13 h-13 rounded-2xl bg-accent text-white/10 border border-[#f05523]/25 flex items-center justify-center text-[#f05523] shrink-0">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[12.5px] text-text-secondary font-bold">We're here 24/7</p>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest pl-1 mt-1">Quick Help</h3>
            <div className="bg-bg-card border border-slate-900 rounded-[24px] overflow-hidden divide-y divide-slate-900/60 shadow-lg">
              {quickHelpTopics.map((topic) => (
                <button key={topic.id} onClick={() => startChatWithTopic(topic.id, topic.title, topic.initialResponse)} className="w-full flex items-center justify-between py-4 px-5 hover:bg-bg-card/15 text-left transition-colors cursor-pointer bg-transparent border-none">
                  <div className="flex items-center gap-3.5">
                    <div className="w-7 h-7 rounded-lg bg-bg-app/50 border border-slate-900 flex items-center justify-center shrink-0">{topic.icon}</div>
                    <span className="text-[13.5px] font-bold text-text-primary tracking-tight">{topic.title}</span>
                  </div>
                  <ChevronRight className="w-4 h-4 text-text-secondary stroke-[2px]" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="pt-2">
            <button onClick={startGeneralSupportChat} className="w-full py-4 bg-accent text-white hover:bg-[#d04417] text-white rounded-[22px] text-sm font-black tracking-wide text-center transition-all shadow-lg active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2.5">
        ...
        <div className="flex flex-col flex-1 bg-bg-card/45 border border-slate-900 rounded-[28px] overflow-hidden h-full shadow-2xl relative">
          <div className="bg-bg-card px-4.5 py-3 border-b border-slate-900 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-[ping_1.5s_infinite]"></div>
              <div>
                <span className="text-[11px] text-text-secondary font-bold block">
            ...
            <span className="text-[9.5px] font-mono text-text-secondary uppercase bg-bg-app px-2 py-0.5 rounded border border-slate-900 font-bold">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3.5 max-h-[350px] min-h-[300px]">
            {chatMessages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col max-w-[85%] ${msg.from === 'DRIVER' ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                <div className={`p-3.5 rounded-2xl text-[12.5px] font-bold leading-relaxed ${msg.from === 'DRIVER' ? 'bg-accent text-white text-white rounded-tr-none' : 'bg-bg-app/60 border border-slate-900 text-text-primary rounded-tl-none'}`}>
                  {msg.message}
                </div>
                <span className="text-[9px] text-text-secondary font-bold mt-1.5 px-1 font-mono">{msg.timestamp}</span>
              </div>
            ))}
            {isTyping && (
              <div className="mr-auto flex flex-col items-start max-w-[50%]">
                <div className="bg-bg-app/60 border border-slate-900 p-3.5 rounded-2xl rounded-tl-none">
                  <span className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-primary text-black rounded-full animate-bounce"></span>
                    <span className="w-1.5 h-1.5 bg-primary text-black rounded-full animate-bounce delay-100"></span>
                    <span className="w-1.5 h-1.5 bg-primary text-black rounded-full animate-bounce delay-200"></span>
                  </span>
                </div>
          ...
          <div className="px-4 py-2 border-t border-slate-900/60 flex gap-2 overflow-x-auto whitespace-nowrap bg-bg-app/15 scrollbar-none">
            ...
            <button onClick={() => { setInputText("How do I cancel active route?"); }} className="px-3 py-1 bg-bg-card/95 border border-slate-850 rounded-full text-[10px] text-text-secondary hover:text-white transition-colors hover:border-slate-700 cursor-pointer font-bold">Cancel active route?</button>
            <button onClick={() => { setInputText("Merchant order is delayed"); }} className="px-3 py-1 bg-bg-card/95 border border-slate-850 rounded-full text-[10px] text-text-secondary hover:text-white transition-colors hover:border-slate-700 cursor-pointer font-bold">Order is delayed</button>
            <button onClick={() => { setInputText("What is payout method Zelle fee?"); }} className="px-3 py-1 bg-bg-card/95 border border-slate-850 rounded-full text-[10px] text-text-secondary hover:text-white transition-colors hover:border-slate-700 cursor-pointer font-bold">How much are fees?</button>
          </div>

          <form onSubmit={handleSendMessage} className="p-3 bg-[#080916] border-t border-slate-900 flex gap-2 items-center">
            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Describe your active challenge..." className="flex-1 bg-bg-app px-4 py-3 rounded-2xl border border-slate-900 text-[12px] font-bold text-white placeholder-slate-500 focus:outline-none focus:border-[#f05523]/50" />
            <button type="submit" disabled={!inputText.trim()} className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all stroke-[2.5px] ${inputText.trim() ? 'bg-accent text-white hover:bg-[#d04417] text-white cursor-pointer active:scale-[0.94]' : 'bg-bg-app text-slate-600 border border-slate-900 cursor-not-allowed'}`}>
