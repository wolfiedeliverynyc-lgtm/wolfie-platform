...
export default function ActiveDeliveryWidget({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadUrl = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/api/v1/uploads` : 'http://localhost:5000/api/v1/uploads';
      const res = await fetch(uploadUrl, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer drv_alpha_001` // dummy token to satisfy backend
        },
        body: formData
      });
      const data = await res.json();
      if (data.url) {
        setSimulatedPhotoUrl(data.url); // update to the remote url
      }
    } catch (err) {
        ...
        {order.status === 'ARRIVED_AT_CUSTOMER' && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="space-y-2">
              ...
              />
              <button
                id="take-delivery-photo"
                onClick={() => fileInputRef.current?.click()}
                disabled={capturing}
                className={`w-full py-2.5 rounded-xl text-xs font-bold border flex items-center justify-center gap-1.5 transition-all ${capturing ? 'bg-slate-950 border-slate-850 text-slate-500' : 'bg-slate-950 hover:bg-slate-850 border-slate-800 text-slate-200 cursor-pointer'}`}
              >
                {capturing ? (
