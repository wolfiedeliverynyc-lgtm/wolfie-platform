...
export function useGPS({ onLocation, batteryAware = true }: GPSOptions = {}) {
  ...
  const smoothPosition = (newLat: number, newLng: number): [number, number] => {
    ...
    // Simple exponential moving average (smoothing factor 0.7)
    const alpha = 0.7
    return [
  ...
  const startSimulation = useCallback(() => {
    // Fallback: same simulation as before
  ...
  const startTracking = useCallback(() => {
    ...
    // Battery-aware interval: low battery = less frequent updates
  ...
  }, [isOnline, onLocation, setCurrentLocation, setDriverHeading, startSimulation])
